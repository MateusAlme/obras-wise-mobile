      ...
      <ProtectedRoute>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm font-medium text-gray-500 dark:text-slate-400">Carregando dashboard...</p>
          </div>
        </div>
      </ProtectedRoute>
    )
    ...
    <ProtectedRoute>
      <AppShell>
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium text-slate-500 dark:text-slate-300 shadow-sm shrink-0">
        ...
        <div className="card-padded mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-bold text-slate-800 dark:text-slate-100">Filtros</span>
              {activeFilters > 0 && (
            ...
            {activeFilters > 0 && (
              <button
                ...
                className="text-xs text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 font-semibold transition-colors flex items-center gap-1"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">Nº da Obra</label>
              <div className="relative">
                <svg className="w-4 h-4 text-gray-400 dark:text-slate-500 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                />
                {searchNumeroObra && (
                  <button onClick={() => setSearchNumeroObra('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">Período</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">Equipe</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">Tipo de Serviço</label>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="stat-card">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{stats.totalObras}</p>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mt-0.5">Total de Obras</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{stats.equipesUnicas} equipe{stats.equipesUnicas !== 1 ? 's' : ''} ativa{stats.equipesUnicas !== 1 ? 's' : ''}</p>
          </div>
          ...
          <div className="stat-card">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{stats.concluidas}</p>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mt-0.5">Concluídas</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{stats.percentualConcluidas}% do total</p>
          </div>
          ...
          <div className="stat-card">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{stats.totalFotos}</p>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mt-0.5">Total de Fotos</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">~{stats.mediaFotosPorObra} por obra</p>
          </div>
          ...
          <div className="stat-card">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{stats.comAtipicidades}</p>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mt-0.5">Atipicidades</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{stats.percentualAtipicidades}% do total</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
          ...
          <div className="card-padded">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Top 5 Equipes</h3>
                <p className="text-xs text-slate-400 dark:text-slate-500">por número de obras</p>
              </div>
            </div>
            <div className="space-y-4">
              {topEquipes.length === 0 ? (
                <p className="text-slate-400 dark:text-slate-500 text-sm text-center py-4">Nenhuma obra no período</p>
              ) : (
                  ...
                  return (
                    <div key={equipe}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs font-bold text-slate-600 dark:text-slate-300">{count} obras</span>
                      </div>
          ...
          <div className="card-padded">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Por Tipo de Serviço</h3>
                <p className="text-xs text-slate-400 dark:text-slate-500">distribuição percentual</p>
              </div>
            </div>
            <div className="space-y-4">
              {distribuicaoServicos.length === 0 ? (
                ...
                distribuicaoServicos.slice(0, 6).map(([servico, count]) => {
                  ...
                  return (
                    <div key={servico}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate mr-4">{servico}</span>
                        <span className="text-xs font-bold text-slate-600 dark:text-slate-300 shrink-0">{count} ({percentage.toFixed(0)}%)</span>
                      </div>
                      <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2">
                        <div
        ...
        <div className="table-wrapper">
          <div className="table-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Estatísticas por Equipe</h3>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{equipesStats.length} equipe{equipesStats.length !== 1 ? 's' : ''} cadastrada{equipesStats.length !== 1 ? 's' : ''}</p>
              </div>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-50 dark:divide-slate-700">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-700">
                {equipesStats.map((s, i) => (
                  <tr key={s.equipe} className="hover:bg-slate-50/60 dark:hover:bg-slate-700/40 transition-colors">
                    <td className="td">
                      <div className="flex items-center gap-3">
                        <div
                        ...
                        <span className="font-medium text-slate-800 dark:text-slate-100">{s.equipe}</span>
                      </div>
                    </td>
                    <td className="td-center font-bold text-slate-900 dark:text-white">{s.totalObras}</td>
                    <td className="td-center">
                      {s.obrasHoje > 0
                        ? <span className="badge-blue">{s.obrasHoje}</span>
                        : <span className="text-gray-300 dark:text-slate-600">—</span>}
                    </td>
                    <td className="td-center text-gray-500 dark:text-slate-300">{s.obrasSemana}</td>
                    <td className="td-center text-gray-500 dark:text-slate-300">{s.obrasMes}</td>
                    <td className="td-center">
                      <span className="text-emerald-600 dark:text-emerald-300 font-medium">{s.totalFotos}</span>
                    </td>
                    <td className="td-center">
                      {s.comAtipicidades > 0
                    ...
                    <td className="td-center text-gray-500 dark:text-slate-300">{s.tiposServico.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {equipesStats.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-sm font-medium text-gray-400 dark:text-slate-400">Nenhuma obra cadastrada</p>
            </div>
          )}
        </div>
      </AppShell>
