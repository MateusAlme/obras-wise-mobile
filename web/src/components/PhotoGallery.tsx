  ...
  return (
    <div className="mb-6">
      {title && (
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-slate-100 flex items-center gap-2">
            ...
            {title}
            <span className="text-sm text-gray-500 dark:text-slate-400 font-normal">({uploadedPhotos.length}{pendingCount > 0 ? `+${pendingCount}⏳` : ''})</span>
          </h3>
          {allowAdd && sectionKey && (
            <div className="flex items-center gap-2">
              <button
                onClick={() => fileInputRef.current?.click()}
                className="px-3 py-2 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 hover:border-blue-500 text-sm font-semibold text-gray-700 dark:text-slate-100 rounded-lg shadow-sm transition-colors"
                disabled={adding}
              >
      ...
      {pendingCount > 0 && !hasPhotos && (
        <div className="mb-3 px-3 py-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 rounded-lg text-sm text-yellow-700 dark:text-yellow-300">
      ...
      {hasPhotos ? (
        ...
        !pendingCount && (
          <div className="rounded-xl border border-dashed border-gray-300 dark:border-slate-500 p-6 text-center text-sm text-gray-500 dark:text-slate-300">
      ...
      {pendingCount > 0 && hasPhotos && (
        <div className="mt-2 px-3 py-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 rounded-lg text-sm text-yellow-700 dark:text-yellow-300">
          +{pendingCount} foto{pendingCount > 1 ? 's' : ''} aguardando sincronização com o servidor.
        </div>
      )}
